...
class TryoutCog(commands.GroupCog, name="tryout"):
    ...
    # TODO: Check if guild has any logs, and create one for it if it does not.
    ...
    def __init__(self, bot: commands.Bot):
        ...
        # self.manager_role = self.logs["Manager Role"]
        # self.tryout_role = self.logs["Tryout Role"]
        # self.admin_roles = self.logs["Admin Roles"]
        

    def check_permissions(self, interaction_user:discord.Member, guild_id):
        ...
        manager_role = self.logs[guild_id]["Manager Role"]
        admin_roles = self.logs[guild_id]["Admin Roles"]
    ...
    async def no_guild_logs(self, interaction: discord.Interaction):
        self.logs[str(interaction.guild_id)] = {"Tryout Role": "", "Manager Role": "", "Admin Roles": [], "Tryouts": {}}
    ...
    def sanity_check_roles(self, guild: discord.Guild):
        guild_logs = self.logs.get(str(guild.id))
    ...
    async def assign_tryout(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not allowed:
            ...
            return
        guild_logs = self.logs[str(interaction.guild.id)]
        user_logs = guild_logs["Tryouts"].get(user.id, {})
        ...
        user_logs["Log"] = f"tryoutData/logs/{user.id}"
        guild = self.bot.get_guild(interaction.guild_id)
        role = guild.get_role(int(guild_logs["Tryout Role"]))
        user.add_roles(role)
        self.logs[str(interaction.guild.id)]["Tryouts"][user.id] = user_logs
        self.save_tryout_logs()
    ...
    async def list_tryouts(self, interaction: discord.Interaction):
        ...
        guild_log = self.logs[str(interaction.guild_id)]
        return_message = "Your tryouts:\n" if not admin else "All tryouts\n"
        for k,v in guild_log["Tryouts"].items():
            if admin:
    ...
    async def transfer_tryout(self, interaction: discord.Interaction, user: discord.Member, manager: discord.Member):
        ...
        if not allowed:
            await interaction.response.send_message("You do not have permission to use this command.", ephemeral=True)
            return
        guild_logs = self.logs[str(interaction.guild_id)]
        ...
        if not user_logs.get("Tryout", False):
            ...
            return
        manager_allowed, _ =self.check_permissions(manager, interaction.guild_id)
    ...
    async def end_tryouts(self, interaction: discord.Interaction, confirm:bool = False):
        ...
        for k,v in guild_logs["Tryouts"]:
            if str(interaction.user.id) == v["Manager"]:
                ...
                self.logs[str(interaction.guild.id)]["Tryouts"][k] = v
        if len(removed_users) == 0:
            await interaction.response.send_message("No tryouts have been ended. Are you sure you are managing any tryouts?", ephemeral=True)
            return
        else:
            self.save_tryout_logs()
    ...
    async def set_tryout_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if not admin:
            ...
            return
        self.logs[str(interaction.guild_id)]["Tryout Role"] = str(role.id)
        self.write_log()
    ...
    async def set_manager_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if not admin:
            await interaction.response.send_message("You do not have permission to use this command. Only admins can use it", ephemeral=True)
            return
        self.logs[str(interaction.guild_id)]["Manager Role"] = str(role.id)
        self.write_log()
    ...
    async def add_admin_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if not interaction.user.guild_permissions.administrator:
            ...
            return
        if str(role.id) in self.logs[str(interaction.guild_id)]["Admin Roles"]:
            await interaction.response.send_message(f"Role <@{role.id}> is already in list of admins", ephemeral=True)
            return
        else:
            self.logs[str(interaction.guild_id)]["Admin Roles"].append(str(role.id))
            self.write_log()
    ...
    async def remove_admin_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("You do not have permission to use this command. Only adminstrators can use it", ephemeral=True)
            return
        if str(role.id) not in self.logs[str(interaction.guild_id)]["Admin Roles"]:
            await interaction.response.send_message(f"Role <@{role.id}> not on list of admins", ephemeral=True)
            return
        else:
            self.logs[str(interaction.guild_id)]["Admin Roles"].remove(str(role.id))
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        return_message += f"See the command descriptions for what they each do. As this is still in beta, then expect a lot of problems. Please report any problems to <@{310153696044515349}>. Enjoy"
        interaction.response.send_message(return_message, ephemeral=True)
